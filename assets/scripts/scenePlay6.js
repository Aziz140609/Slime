var scenePlay6 = new Phaser.Class({
  ...
  create: function () {
    ...
    spawnEnemy(225, 150, 1); // Musuh 1
    spawnEnemy(500, 50, 3); // Musuh 2
  ...
  update: function () {
    ...
    if (this.player.x > 672) {
      this.registry.set('playerSpawn', { x: 20, y: this.player.y, targetScene: "scenePlay7" });
      this.registry.set('cameFrom_scenePlay7', this.scene.key);
      this.scene.start("scenePlay7");
    }

    if (this.scene.key === "scenePlay6" && this.player.x < 10) {
      this.player.x = 10;
    }

    if (this.scene.key !== "scenePlay" && this.scene.key !== "scenePlay6" && this.player.x < 0) {
